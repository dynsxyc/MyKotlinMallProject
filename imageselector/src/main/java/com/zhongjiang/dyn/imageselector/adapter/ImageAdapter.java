...
public class ImageAdapter extends RecyclerView.Adapter<ImageAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        setItemSelect(holder, containsImage(image));
        //点击选中/取消选中图片
    ...
    private boolean containsImage(Image image){
        ...
        Iterator<Image> imageIterator = mSelectImages.iterator();
        while (imageIterator.hasNext()) {
           Image mSelectImage = imageIterator.next();
            if (mSelectImage.equals(image)){
                contains = true;
            }
            if (!ImageSelectorFileUtils.fileIsExists(mSelectImage.getPath())){
